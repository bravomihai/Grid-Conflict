...
//
// Helper: map row encoding/decoding consistent cu codul original:
// 0..25 -> 'A'..'Z', 26..51 -> 'a'..'z'
//
...
//
// encode/decode: păstrează formatul original (prefix 'o' pentru obiecte)
// encode: board -> string
// decode: string -> board
//
void encode(int H, int W, const std::vector<std::vector<char>>& board, std::string &out) {
    ...
    for (int r = 0; r < H; ++r) {
        for (int c = 0; c < W; ++c) {
            char ch = board[r][c];
            if (!(std::isdigit(static_cast<unsigned char>(ch)) || std::isupper(static_cast<unsigned char>(ch)) || std::islower(static_cast<unsigned char>(ch))))
                continue;
            if (std::isdigit(static_cast<unsigned char>(ch))) out.push_back('o');
            out.push_back(ch);
            out.push_back(' ');
            out.push_back(indexToRowChar(r));
            int col = c + 1;
            if (col >= 10) out.push_back(char('0' + (col / 10) % 10));
...
void decode(int H, int W, std::vector<std::vector<char>>& board, const std::string &s) {
    ...
    while (i < n) {
        ...
        if (s[i] == ' ') { ++i; continue; }
        bool is_obj = false;
        if (s[i] == 'o') { is_obj = true; ++i; if (i >= n) break; }
...
static inline int add_c(std::string &s, int start, int len) {
    if (start < 0 || len <= 0 || start > (int)s.size()) return -1;
...
//
// closest_point and closest_valid_point: preserve original movement heuristics
//
static point closest_point(char row_c, int col, char np_row_c, int np_col, int s, int s2) {
    // convert to indices
    ...
    while (cur_row < row && s) { cur_row++; --s; }
    // going right
    while (cur_col < col && s) { ++cur_col; --s; }
    // going up
    while (cur_row > row && s) { --cur_row; --s; }
    // going left
    while (cur_col > col && s) { --cur_col; --s; }
    ...
    // s2 attempts
    while (cur_col < col && s2) { ++cur_col; --s2; }
    while (cur_col > col && s2) { --cur_col; --s2; }
    while (cur_row < row && s2) { ++cur_row; --s2; }
    while (cur_row > row && s2) { --cur_row; --s2; }
...
static point closest_valid_point(char row, int col, char np_row, int np_col, int s, const std::string &str, int H, int W) {
    point closest = closest_point(row, col, np_row, np_col, s, 0);
    while (!([&](const point &p)->bool {
        // valid_position: in map and not already occupied according to encoded string
        int ridx = rowCharToIndex(p.row);
        if (!(ridx >= 0 && ridx < H && p.col > 0 && p.col <= W)) return false;
        // build simple substr like original
        std::string sub;
        sub.push_back(p.row);
        if (p.col > 9) sub.push_back(char('0' + (p.col / 10) % 10));
...
static bool isobject_index(int i, const std::string &s) {
    ...
    if (i - 2 >= 0) {
        char prev = s[i - 2];
        if (prev == 'm' || prev == 'A' || prev == 'B') return false;
    }
    return true;
}

//
// end_round and move_player adapted to std::string
//
static void end_round_state(game_state &dest, const game_state &src, Move &m, int np, int otherp) {
    m.type = 'p';
    m.torow = '.';
    m.tocol = 0;
    dest = src;
    // nothing else: stats already copied by assignment
}

static bool move_player_apply(game_state &dest, const game_state &src, Move &m, char row, int col, char np_row, int np_col, char next_player) {
    m.type = 'm';
    m.torow = row;
    m.tocol = (short)col;
    dest = src;
    ...
    if (pos >= dest.s.size()) return false;
    dest.s[pos++] = row;
    if (col > 9) {
        if (pos >= dest.s.size()) dest.s.push_back('0');
...
//
// next_states: transliterare fidelă din C, dar cu std::vector & std::string
//
                ...
                std::vector<Move>& moves)
{
    ngs.clear();
    moves.clear();
    // reserve up to 30 like original
    ngs.resize(30);
    moves.resize(30);
    ...
    for (int i = 0; i < (int)gs.s.size(); ++i) {
        if (gs.s[i] == 'm' && i + 1 < (int)gs.s.size() && gs.s[i+1] == ' ') ++m;
        if (isobject_index(i, gs.s)) ++o;
    }
    ...
    if (dtop == 1) {
        if (gs.players[np].s >= 10) {
            ...
            ngs[state] = gs;
            // damage
            short damage = std::max<short>(0, gs.players[np].A - gs.players[otherp].D);
            ...
            for (int dir = 0; dir < 4; ++dir) {
                ...
                if (in_bounds) {
                    std::string sub; sub.push_back(nr);
                    if (nc > 9) sub.push_back(char('0' + (nc / 10) % 10));
                ...
                if (in_bounds && not_occupied) {
                    ...
                    if (dist <= gs.players[np].s) {
                        ...
                        Move mm;
                        game_state dest = gs;
                        if (move_player_apply(dest, gs, mm, nr, nc, np_row, np_col, next_player)) {
                            dest.players[np].s = (short)(gs.players[np].s - dist);
                            ngs[state] = std::move(dest);
                            mm.type = 'm';
                            moves[state] = mm;
                            ++state;
                            break; // dir = 4 in original
                        }
                    }
                }
            }
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            if (in_bounds && not_occupied) {
                ...
                if (dist + 10 <= gs.players[np].s) {
                    ...
                    point closest = closest_valid_point(nr, nc, np_row, np_col, gs.players[np].s, gs.s, H, W);
                    int d = distance_rc(closest.row, closest.col, np_row, np_col);
                    std::string sub; sub.push_back(closest.row);
                    if (closest.col > 9) sub.push_back(char('0' + (closest.col / 10) % 10));
                    sub.push_back(char('0' + (closest.col % 10)));
                    bool val = (gs.s.find(sub) == std::string::npos);
                    if (val && d <= gs.players[np].s) {
                        Move mm; game_state dest = gs;
                        if (move_player_apply(dest, gs, mm, closest.row, closest.col, np_row, np_col, next_player)) {
                            dest.players[np].s = 0;
                            ngs[state] = std::move(dest);
                            mm.type = 'm';
                            moves[state] = mm;
    ...
    while (state <= o + 1) {
        // find next object
        while (i < (int)gs.s.size() && !isobject_index(i, gs.s)) ++i;
        if (i >= (int)gs.s.size()) break;
        int nr_o = (int)(gs.s[i+1] - '0');
        i += 3;
        int start = i - 3, end = i + 1;
        while (end - 1 < (int)gs.s.size() && gs.s[end - 1] != ' ' && gs.s[end - 1] != '\0') ++end;
        ...
        if (dist <= gs.players[np].s) {
            ...
            game_state dest = gs;
            if (delete_c(dest.s, start, end - start) == -1) return -1;
            Move mm;
            mm.type = 'm';
            mm.torow = orow;
            mm.tocol = (short)ocol;
            // update player's pos in dest.s
            if (!move_player_apply(dest, gs, mm, orow, ocol, np_row, np_col, next_player)) return -1;
            // update stats
            dest.players[np].s = (short)(gs.players[np].s - dist);
            ...
            point closest = closest_valid_point(orow, ocol, np_row, np_col, gs.players[np].s, gs.s, H, W);
            std::string sub; sub.push_back(closest.row);
            if (closest.col > 9) sub.push_back(char('0' + (closest.col / 10) % 10));
            sub.push_back(char('0' + (closest.col % 10)));
    ...
    while (state <= o + m + 1) {
        // find monster "m "
        while (i < (int)gs.s.size() && !(gs.s[i] == 'm' && i + 1 < (int)gs.s.size() && gs.s[i+1] == ' ')) ++i;
        if (i >= (int)gs.s.size()) break;
        i += 2;
            ...
            for (int dir = 0; dir < 4; ++dir) {
                ...
                if (in_bounds && gs.s.find(sub) == std::string::npos) {
                    ...
                    if (dist <= gs.players[np].s) {
                        ...
                        point closest = closest_valid_point(nr, nc, np_row, np_col, gs.players[np].s, gs.s, H, W);
                        std::string subs; subs.push_back(closest.row);
                        if (closest.col > 9) subs.push_back(char('0' + (closest.col / 10) % 10));
    ...
    // final end_round as in original
    end_round_state(ngs[state], gs, moves[state], np, otherp);
    ++state;
    ngs.resize(state);
    moves.resize(state);
...
//
// static_eval and game_over (same heuristics as original)
//
...
static int static_eval(const game_state &gs, char next_player) {
    ...
    if (game_over_check(gs) && gs.players[player_index].H > 0) return INF;
    if (game_over_check(gs) && gs.players[player_index].H <= 0) return -INF;
    int score = (int)gs.players[player_index].H + gs.players[player_index].A + gs.players[player_index].D + gs.players[player_index].S
                - ((int)gs.players[opp_index].H + gs.players[opp_index].A + gs.players[opp_index].D + gs.players[opp_index].S);
    return score;
}
...
static int minimax_search(const game_state &gs, int depth, bool is_maximizing,
                          int H, int W, char next_player, const std::vector<item>& items,
                          int &alpha, int &beta)
{
    if (depth == 0 || game_over_check(gs)) return static_eval(gs, next_player);
    ...
    if (is_maximizing) {
        ...
        for (int i = 0; i < nos; ++i) {
            ...
            if (moves[i].type == 'p' || ngs[i].players[player_index].s == 0) {
                eval = minimax_search(ngs[i], depth - 1, false, H, W, next_player, items, alpha, beta);
            } else {
                eval = minimax_search(ngs[i], depth, true, H, W, next_player, items, alpha, beta);
            }
        ...
        for (int i = 0; i < nos; ++i) {
            ...
            if (moves[i].type == 'p' || ngs[i].players[opp_index].s == 0) {
                eval = minimax_search(ngs[i], depth - 1, true, H, W, next_player, items, alpha, beta);
            } else {
                eval = minimax_search(ngs[i], depth, false, H, W, next_player, items, alpha, beta);
            }
...
Move best_move(const char* file_name) {
    Move nullm; nullm.type = 'p'; nullm.torow = '.'; nullm.tocol = 0;
    std::ifstream fin(file_name);
    if (!fin.is_open()) return nullm;
    ...
    if (!gs.s.empty() && gs.s.back() == '\r') gs.s.pop_back();
    ...
    if (nr_of_states <= 0) return nullm;

    int best_index = 0;
    int best_score = -INF;
    int alpha = -INF, beta = INF;
    int depth = 6; // realistic depth; original used 10, but keep moderate for time
    for (int i = 0; i < nr_of_states; ++i) {
        bool is_maximizing = (moves[i].type == 'p') ? false : true; // original behavior
        int a = alpha, b = beta;
        int score = minimax_search(ngs[i], depth, is_maximizing, H, W, next_player, items, a, b);
